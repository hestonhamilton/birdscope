...
productname="Pan-Tilt HAT" # the name of the product to install
scriptname="pantilthat" # the name of this script
...
debpackage="pantilthat" # the name of the package in apt repo
piplibname="pantilthat" # the name of the lib in pip repo
...
localdir="pantilthat" # the name of the dir for copy of resources
gitreponame="pantilt-hat" # the name of the git project repo
...
examplesdep=( "picamera" ) # list of python modules required by examples
...
# pypi repo install
    ...
    if [ -n "$VIRTUAL_ENV" ]; then
        echo "Installing $piplibname into virtual environment..."
        if ! $PIP3_BIN install --upgrade "$piplibname"; then
            warning "Python 3 library install failed!"
            echo "If problems persist, visit forums.pimoroni.com for support"
            exit 1
        fi
    else
        echo "Installing $piplibname system-wide (using --break-system-packages)..."
        ...
        for moredep in ${examplesdep[@]}; do
            ...
            if [ -f "$(which python3)" ] && apt_pkg_req "python3-$moredep"; then
                if ! apt_pkg_install "python3-$moredep"; then
                	if [ -n "$VIRTUAL_ENV" ]; then
			    echo "Installing $moredep into virtual environment..."
    			if ! $PIP3_BIN install "$moredep"; then
        			warning "Python 3 library install failed for $moredep!"
        			FAILED_PKG=true
    			fi
		else
		    	if ! sudo -H $PIP3_BIN install "$moredep" --break-system-packages; then
        			warning "Python 3 library install failed for $moredep!"
        			FAILED_PKG=true
    			fi
	    fi
